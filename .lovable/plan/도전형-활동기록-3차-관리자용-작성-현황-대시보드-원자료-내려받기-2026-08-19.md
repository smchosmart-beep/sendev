...
- **ZIP** — 팀마다 `팀이름/활동기록.md` 한 개씩. 기존의 `src/lib/readme-template.ts`를 그대로 쓰지 않고, 활동기록 데이터에 맞춰 새 템플릿 함수(`buildRecordReadme`)를 작성합니다. 섹션은 최종결과물 11개 항목, 반복행(핵심기능/사용흐름/한계/계획/제작자, 과정, 개발, 점검), 개인 후기 순으로 구성합니다. ZIP 생성은 `await import("jszip")`로 동적 로딩합니다. 팀 이름에 들어간 특수문자·공백·슬래시 등은 파일 경로 오류를 방지하기 위해 안전한 문자로 정규화(sanitize)하여 폴더명을 만듭니다.
