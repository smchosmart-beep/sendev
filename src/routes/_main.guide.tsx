  ...
  return (
    <div className="space-y-8">
      ...
      <Section id="password-diff" icon={KeySquare} title="닉네임 비밀번호 사용 안내">
        ...
        <ul className="ml-5 list-disc space-y-2">
          ...
          <li>
            ...
            여러 번 평가하거나, 다른 사람이 내 닉네임을 도용해 평가하는 것을 방지합니다. PC에서는 별을 눌러 점수를
            매기고, 모바일에서는 −/＋ 버튼과 숫자 입력으로 점수를 매겨요.
          </li>
        </ul>
        <p>
        ...
        <p>
          <strong>평가 결과(평균 점수)는 아무나 볼 수 없습니다.</strong> 산출물 글의 평가 요약은
          ① 관리자, ② 평가자 명단을 쓰는 게시판이면 명단에 등록된 닉네임으로 닉네임 비밀번호 확인을 마친 분,
          ③ 명단을 쓰지 않는 게시판이면 그 글에 이미 평가를 제출한 본인에게만 보입니다.
          닉네임과 닉네임 비밀번호를 입력하면 권한이 확인되어 요약이 열립니다.
          관리자가 <strong>평가 결과 전체 공개</strong> 스위치를 켜면 그때부터 모두에게 평균 점수가 공개됩니다.
        </p>
        <p>
      ...
      <Section id="badge" icon={Award} title="배지 제도">
        ...
        <p>
          닉네임 변경은 <strong>마이페이지에서 본인이 직접</strong> 할 수 있고, 사정이 있을 때는{" "}
          <strong>관리자가 대신 변경</strong>해 줄 수도 있습니다(관리자 → 사용자 프로필 → 연필 아이콘).
          어느 쪽이든 글·댓글·좋아요·배지·레벨·투표·평가·활동 기록·읽음 표시가 새 닉네임으로 함께
          옮겨집니다.
        </p>
        <p>
          다만 닉네임과 비밀번호는 <strong>기기(브라우저)에도 저장</strong>되어 있습니다. 관리자가 대신
          변경한 경우에는 본인이 사용하던 기기에서 닉네임을 <strong>새 이름으로 바꾸고 기존 비밀번호로 다시
          확인</strong>해야 합니다. 그렇지 않으면 예전 이름으로 글이 올라가 활동 기록이 둘로 나뉠 수 있습니다.
        </p>
        <p>
          예전 이름으로 활동 기록만 남아 있는 경우에는 관리자 → 사용자 프로필 목록에{" "}
          <strong>(미등록)</strong> 표시로 나타납니다. 이때 <strong>합치기</strong> 버튼으로 지금 쓰는
          닉네임에 기록을 합칠 수 있고, 투표 수 등은 그대로 유지됩니다.
        </p>
      </Section>
      ...
      <Section id="menus" icon={LayoutGrid} title="메뉴 안내">
        ...
        <p className="mt-3">
          <strong>메뉴 접기</strong>: 상단 메뉴 오른쪽의 화살표 버튼을 누르면 데스크탑 화면에서 메뉴를 접어
          숨길 수 있습니다. 다시 누르면 펼쳐지며, 접힘/펼침 상태는 기기별로 기억됩니다.
        </p>
        <p className="mt-3">
      ...
      <Section id="posting" icon={LayoutGrid} title="글쓰기와 공지">
        ...
        <ul className="ml-5 list-disc space-y-2">
          ...
          <li><strong>산출물 게시판 투표</strong>: 관리자로 로그인하면 산출물 목록 위에 투표 제어판이 나타나고, <strong>[투표 시작]</strong>을 누르며 1인당 최대 투표 수와 선발 정원을 정할 수 있습니다. 투표가 시작되면 산출물 목록이 투표 화면으로 바뀌며, <strong>그 게시판에 산출물을 등록한 닉네임</strong>만 투표할 수 있습니다(닉네임·닉네임 비밀번호 등록 필요). 본인이 올린 산출물에는 투표할 수 없습니다. 투표 현황판·저장 방식·결선(재)투표·초기화는 투표 게시판과 동일하게 동작하며, <strong>[투표 종료]</strong>를 누르면 득표순으로 정렬되어 순위 배지와 선발 표시가 나타납니다. 산출물 투표에서는 작성자(팀명)가 <strong>처음부터 계속 공개</strong>됩니다. 초기화 후에는 원래의 산출물 목록 화면으로 돌아갑니다.</li>
          <li><strong>목록 카드 표시 방식</strong>: 고정 게시글과 일반게시판 목록 카드는 화면 크기에 따라 다르게 배치됩니다. <strong>모바일</strong>에서는 제목이 한 줄 전체를 차지해 최대 두 줄까지 보이고, 바로 아래 줄에 <strong>작성자 이름 · 레벨 · 배지</strong>와 댓글 수가 표시됩니다(조회수는 숨김). <strong>PC</strong>에서는 왼쪽에 제목, 오른쪽에 조회수 · 댓글 수 · 작성자 정보가 한 줄로 표시됩니다.</li>
          ...
          <li><strong>투표 게시판</strong>: 후보 글을 올리고 참여자들이 투표하는 게시판입니다. 글은 <strong>한 닉네임당 1개</strong>만 등록할 수 있고, 등록한 글이 곧 투표 후보가 됩니다. 글을 올릴 때는 <strong>제목 없이 본문만</strong> 작성하며, 대표 링크 입력칸도 없습니다. 목록 카드에는 <strong>본문 내용</strong>이 그대로 보입니다. 닉네임이 투표 판단에 영향을 주지 않도록 <strong>최종 결선까지 모두 끝나고 관리자가 [최종 결과·닉네임 공개]를 누르기 전까지 작성자는 '익명'으로 표시</strong>되며, 관리자도 그 전까지는 확인할 수 없습니다(목록·검색 결과·글 상세 모두 동일). 동점으로 결선이 남아 있으면 공개 버튼이 나타나지 않고, 결선 시작·취소·초기화를 하면 공개 상태가 자동으로 다시 익명으로 돌아갑니다. 필요하면 관리자가 <strong>[닉네임 다시 비공개]</strong>로 되돌릴 수도 있습니다. 목록은 <strong>보드형 카드</strong>로 한 화면에 최대 <strong>36개</strong>까지 보이며 그 이상은 페이지로 넘겨 봅니다. <strong>투표 권한</strong>은 그 게시판의 비밀번호를 알고 들어온 사람 누구나 가지며, 투표하려면 <strong>닉네임과 닉네임 비밀번호</strong>가 등록되어 있어야 합니다(메뉴에서 등록). 투표가 <strong>진행 중일 때는 득표수가 모두 비공개</strong>이고 본인이 어디에 투표했는지만 카드에 표시됩니다. <strong>관리자</strong>는 목록 위 제어판에서 현재 <strong>1인당 최대 투표 수</strong>를 확인하고, <strong>투표 시작</strong>을 누르면 비밀번호 입력 창에서 1인당 최대 투표 수를 다시 확인한 후 시작할 수 있습니다. <strong>투표 종료</strong>와 <strong>초기화</strong> 역시 관리자 비밀번호 입력이 필요하며, 투표 종료를 누르면 그 순간 모든 참여자에게 <strong>득표수와 순위</strong>가 한 번에 공개됩니다(1분 이내 자동 반영). <strong>초기화</strong>를 누르면 그 게시판의 표가 모두 삭제됩니다. 투표는 <strong>후보를 고른 뒤 [투표 저장]을 눌러야</strong> 반영되며, <strong>정해진 표 수를 정확히 모두 채워야만</strong> 저장할 수 있습니다(예: 3표면 반드시 3개 선택). <strong>본인이 쓴 글에는 투표할 수 없고</strong>, 그 카드는 '내 글'로 표시되어 선택되지 않습니다. 본인 글을 뺀 후보가 정해진 표 수보다 적으면 그 개수만큼만 채우면 저장됩니다. 저장한 뒤에도 <strong>종료 전까지는 선택을 바꿔 다시 저장</strong>할 수 있습니다. <strong>선발 정원과 결선(재)투표</strong>: 투표를 시작할 때 관리자가 <strong>선발 정원</strong>(최종 몇 팀을 뽑을지)을 함께 정합니다. 종료 후 정원 경계선에서 동점이 생기면 결과 위에 <strong>“○팀 확정, 남은 자리 △개를 □팀이 동점으로 다투는 중”</strong> 안내가 뜨고, 관리자 제어판에 <strong>[결선 투표 시작]</strong> 버튼이 나타납니다. 결선을 시작하면 확정된 팀은 그대로 유지되고 <strong>동점 후보만</strong> 후보로 남아 다시 투표가 열리며, 이때 <strong>1인당 선택 수를 다시 지정</strong>할 수 있습니다(기본값은 남은 자리 수). 1차 표는 지워지지 않고 라운드별로 따로 집계되며, 결선 진행 중에는 득표수가 다시 비공개가 됩니다. 결선이 <strong>종료되면 목록에 이전 라운드에서 확정된 팀과 이번 결선 후보가 함께 표시</strong>되어 최종 선발 명단을 한 화면에서 볼 수 있습니다(확정 팀에는 '이전 라운드 확정' 표시와 그 라운드의 득표수가 함께 보입니다). <strong>최종 순위</strong>는 1차에서 먼저 확정된 팀부터 1위, 2위… 순으로 매겨지고, 결선에서 추가로 선발된 팀은 그 다음 번호로 순위가 이어집니다. 결선이 끝난 뒤에는 <strong>선발되지 않은 게시글도 그 아래에 순위와 득표수와 함께 모두 표시</strong>되어 전체 순위를 확인할 수 있습니다(득표가 0인 글은 목록에는 보이지만 순위 표시는 없습니다). 관리자에게는 각 카드에 <strong>그 글이 표를 받은 라운드의 투표자 명단</strong>이 함께 보입니다. 결선에서도 동점이면 같은 방식으로 다음 라운드를 열 수 있고, <strong>[결선 취소]</strong>를 누르면 현재 결선 라운드의 표만 지우고 직전 결과로 돌아갑니다. <strong>투표 현황판</strong>: 투표가 진행 중일 때는 참여자 닉네임별 <strong>투표 완료 여부</strong>가 보입니다. 화면이 넓은 PC(가로 1536px 이상)에서는 본문 좌우 여백에 명단이 반씩 나뉘어 고정 표시되고, 그보다 좁은 화면과 모바일에서는 투표 목록 위의 <strong>접이식 “투표 현황 n / m 완료”</strong> 패널을 눌러 펼쳐 볼 수 있습니다. 명단은 그 게시판에 글을 올린 작성자 닉네임을 <strong>가나다순</strong>으로만 정렬해 보여 주므로 어떤 글의 작성자인지 알 수 없고, <strong>누가 무엇에 투표했는지는 표시되지 않습니다</strong>(완료 여부만). 현황판은 모두에게 공개되며 투표가 종료되면 사라집니다.</li>
          <li><strong>활동기록 – 두 가지 유형(도전형·성장형)</strong>: 활동기록 게시판은 관리자가 <strong>도전형</strong>과 <strong>성장형</strong> 중 하나로 정해 둡니다. <strong>도전형</strong>은 <strong>팀 단위 8단계</strong> 기록이고, <strong>성장형</strong>은 <strong>개인 1인 전용 7단계</strong> 기록입니다. 성장형은 팀원 추가 없이 <strong>작성자 본인과 관리자만</strong> 편집할 수 있으며, 단계는 <strong>01 나의 프로젝트 → 02 문제와 해결 → 03 결과물 → 04 검토 및 개선 → 05 성찰과 성장 → 06 README → 07 성장 사례집</strong>입니다. <strong>01</strong>에서는 프로젝트명·한 줄 소개·주 사용자·문제 영역·결과물 형태를 고르고, <strong>02</strong>에서는 해결하고 싶은 문제와 그 근거, 해결 방법, 기대하는 변화를 적습니다. <strong>03</strong>에서는 배포 주소·GitHub 저장소 주소·완성 상태·사용한 도구와 함께 <strong>핵심 기능(최대 3줄)·사용 흐름(최대 3단계)·대표 이미지 1장</strong>(2MB 이하, 90도 회전 가능)을 정리합니다. <strong>04 검토 및 개선</strong>은 <strong>1차 자기 점검 · 2차 AI 점검 · 3차 동료 점검</strong> 세 탭으로 나뉘고, 점검마다 수정 기록을 남기면 위쪽에 <strong>n / 3 점검</strong>으로 표시됩니다. <strong>3차 동료 점검의 짝은 무작위로 한 번만 배정되고, 그 뒤에는 바뀌지 않습니다.</strong> 짝의 배포 주소를 열어 보고 돌아와도 같은 짝이 그대로입니다. <strong>05 성찰과 성장</strong>에서는 가장 어려웠던 점과 해결 방법, AI에 맡긴 일과 내가 직접 판단한 일, <strong>개인정보 처리 여부</strong>(세 가지 중 하나 선택), 중요하게 생각한 윤리 원칙(복수 선택), 실천 약속, 새롭게 할 수 있게 된 것, 다음에 보완할 것을 씁니다. 04에서 '다음에 고침'으로 분류한 항목은 <strong>[04에서 가져오기]</strong> 버튼으로 '다음에 보완하고 싶은 것'에 이어 붙일 수 있습니다. 입력은 <strong>약 1초 뒤 자동 저장</strong>되고(개인정보 처리 여부 선택도 자동 저장), 위쪽 단계 버튼에 <strong>작성 수 / 필수 수</strong> 배지와 완료 체크가 표시됩니다. <strong>06·07</strong>에서는 README 복사와 A4 사례집 인쇄·PDF 저장을 할 수 있습니다. 다른 기기나 브라우저에서 기록을 열면 잠긴 상태로 보이는데, 기록 화면 위쪽에서 <strong>내 닉네임과 닉네임 비밀번호로 본인 확인</strong>을 하면 바로 열립니다(비밀번호를 잊었다면 마이페이지의 비밀번호 찾기 이용). 관리자는 <strong>관리자 &gt; 활동기록 현황</strong>에서 상단의 <strong>도전형 / 성장형</strong> 탭으로 유형을 나눠 진행률을 확인하고 엑셀로 내려받습니다.</li>
          <li><strong>활동기록 게시판</strong>: 팀이 한 학기(또는 한 프로젝트)의 과정을 하나의 기록에 함께 정리하는 게시판입니다. <strong>팀당 기록은 1개</strong>이며, 한 게시판에서 한 사람은 <strong>한 팀에만</strong> 속할 수 있습니다. 이미 팀원으로 등록된 사람이 새 기록을 만들려고 하면 새로 만들지 않고 <strong>기존 기록으로 이동</strong>합니다. 기록을 만든 뒤 <strong>팀원 닉네임을 추가</strong>하면 팀원 모두가 같은 기록을 <strong>함께 편집</strong>할 수 있습니다(각자 자기 닉네임과 닉네임 비밀번호로 인증, 메뉴 &gt; 내 정보에 저장해 두면 자동 적용). 활동기록 글은 <strong>다른 카테고리로 이동할 수 없고</strong> 연재(다음 편) 연결도 하지 않습니다. 정리한 내용은 <strong>07 README 출력 단계에서 바로</strong> <strong>README.md 또는 ZIP(README + 대표 이미지)</strong>으로 내려받을 수 있습니다. 게시판에서 활동기록을 켜고 이름을 바꾸는 것은 <strong>관리자 &gt; 카테고리 관리</strong>에서 합니다.</li>
          <li><strong>활동기록 – 단계별 작성 순서</strong>: 기록은 <strong>01 팀 공통정보 → 02 문제 정의 과정 → 03 최종 결과물 → 04 개발·교육적 점검 → 05 개인 후기·소회 → 06 윤리 설문 → 07 README 출력 → 08 사례집 출력</strong>의 8단계로 나뉘어 있고, 위쪽 단계 버튼이나 아래 [이전 단계]·[다음 단계] 버튼으로 오갈 수 있습니다. 각 단계는 따로 저장되므로 <strong>한 번에 다 쓰지 않아도 됩니다</strong>. 각 입력칸에 흐리게 보이는 <strong>회색 예시 문구</strong>는 어떻게 쓰면 되는지 알려 주는 참고용이며, <strong>실제로 저장되지 않습니다</strong>(직접 입력한 내용만 저장돼요). <strong>01 팀 공통정보</strong>에서는 팀원을 추가·삭제하고 각자의 <strong>소속과 역할</strong>을 적습니다. <strong>02 문제 정의 과정</strong>은 회의·인터뷰 같은 <strong>기록 종류</strong>를 고르고 언제·어디서, 나눈 이야기, 그래서 정한 것을 줄 단위로 남깁니다. <strong>모바일에서는 기록 종류를 드롭다운 메뉴로, PC에서는 탭 버튼으로 고릅니다.</strong> 각 줄에는 <strong>관련 링크</strong>(회의록·설문 등 주소 한 줄)와 <strong>관련 파일</strong>(최대 3개, 개당 10MB)을 함께 붙일 수 있습니다. 파일은 고르는 즉시 올라가고 파일명 칩으로 보이며, 칩을 누르면 내려받고 X로 뺄 수 있습니다. 링크·파일을 바꾼 뒤에는 그 줄의 [저장]을 눌러야 확정됩니다. 다만 <strong>기록 종류(탭)에 따라 입력 양식이 완전히 달라집니다</strong>. 탭은 <strong>[1. 페르소나] · [2. 고객 여정 맵] · [3. 인터뷰 기록] · [4. 회고&amp;문제정의] · [5. 종이 프로토타입 영상 콘티]</strong> 다섯 가지이며, 번호 순서대로 작성하면 문제 재정의가 완성되도록 짜여 있습니다. <strong>[1. 페르소나]</strong>는 페르소나 이미지 한 장만 첨부하고, <strong>[2. 고객 여정 맵]</strong>은 여정 맵 이미지 한 장만 첨부합니다. <strong>[3. 인터뷰 기록]</strong>은 <strong>인터뷰 개요 · 질문과 답변 · 관찰 메모</strong> 세 칸으로 되어 있고, <strong>이 탭만 팀원 여러 명이 각자 여러 건을 추가</strong>할 수 있습니다([+ 인터뷰 기록 추가] 버튼). 각 인터뷰 줄 위에는 <strong>[인터뷰 1] 머리말과 기록한 사람</strong> 칸이 함께 보여 누가 쓴 기록인지 구분됩니다. <strong>[4. 회고&amp;문제정의]</strong>는 <strong>가설과 일치 · 불일치 · 새로 알게 된 내용</strong>에 더해 미션의 최종 산출물인 <strong>문제 재정의</strong> 칸이 있고, 관련 링크·파일도 붙일 수 있습니다. <strong>[5. 종이 프로토타입 영상 콘티]</strong>는 <strong>유튜브 영상 링크만</strong> 제출합니다. 콘티는 위쪽 <strong>템플릿 안내</strong>를 참고해 따로 작성하고, 화면에는 링크 한 줄만 남깁니다(링크 오른쪽 <strong>재생 버튼</strong>으로 새 탭에서 바로 볼 수 있어요). 모든 탭에는 위쪽에 <strong>템플릿 안내 블록</strong>과 <strong>참고 팁</strong>이 보이며, 오른쪽 <strong>[복사]</strong> 버튼으로 양식을 클립보드에 담아 쓸 수 있습니다. 이미지는 점선 영역에 <strong>끌어다 놓거나 클릭</strong>해서 첨부하고, 올리면 본문 너비에 맞춰 크게 미리보기로 보입니다. 긴 칸은 칸당 최대 8,000자까지 쓸 수 있습니다. 새 줄을 추가하는 버튼은 <strong>[3. 인터뷰 기록] 탭에서만</strong> 나타나고, 나머지 탭은 팀당 한 벌만 작성합니다. 탭을 열었는데 아직 기록이 없으면 편집 권한이 있을 때 <strong>빈 양식 한 벌이 바로 펼쳐집니다</strong>. 내용을 채우고 [저장]을 눌러야 실제 기록으로 남습니다. <strong>[+ 추가] 버튼을 누르면 빈 줄이 곧바로 저장되지 않고, <strong>저장 전</strong> 표시가 붙은 점선 양식이 화면에만 생깁니다</strong>. 내용을 적고 [저장]을 눌러야 서버에 반영되며, [취소]를 누르거나 새로고침하면 저장하지 않은 양식은 사라집니다. README·사례집 출력에서도 <strong>1 → 5 탭 순서</strong>로 실립니다. 내용이 하나도 없는 행은 README·사례집 출력과 작성 현황 계산에서 자동으로 빠집니다. <strong>03 최종 결과물</strong>은 한눈에 보기·문제와 해결·사용과 배포·기술 구성·라이선스 항목으로 이루어져 있고, 입력하면 <strong>약 1초 뒤 자동 저장</strong>되며 오른쪽 위에 저장 상태가 표시됩니다. <strong>결과물 형태(웹앱·모바일앱·데스크탑 앱·챗봇·AI 도구·자동화 도구·수업자료·콘텐츠·/skill)는 여러 개를 함께 선택</strong>할 수 있고, 다시 누르면 해제됩니다. 같은 단계의 <strong>핵심 기능 · 사용 흐름 · 현재 한계 · 다음 계획</strong>은 줄 단위로 추가·수정·삭제하고 [저장] 버튼으로 각 줄만 저장합니다. <strong>대표 이미지는 파일 업로드로 등록</strong>합니다([이미지 선택] 버튼으로 파일을 고르면 자동으로 올라가고 미리보기가 보이며, [제거]로 비울 수 있습니다). <strong>환경변수는 이름만</strong> 적고 실제 값(비밀키)은 절대 입력하지 마세요. <strong>제작자와 담당은 03에서 따로 적지 않습니다</strong> — 01 팀 공통정보의 팀원 이름·소속·역할이 그대로 README·사례집의 제작자 항목으로 쓰여요.</li>
          <li><strong>활동기록 – README 출력</strong>: <strong>07 README 출력</strong> 단계에서는 지금까지 쓴 내용으로 만들어진 README를 바로 확인할 수 있습니다. <strong>[README 구성]</strong> 탭은 최종 README를 이루는 <strong>10개 블록</strong>(프로젝트명과 한 줄 설명, 대표 화면과 링크, 해결한 문제, 핵심 기능, 사용 흐름, 기술 스택과 실행, 작동 범위와 한계·다음 계획, 교육 현장 주의사항, 제작자와 라이선스, 교사 개발자 윤리 자가점검)의 <strong>미작성 · 작성중 · 작성완료</strong> 세 단계 상태를 보여 주고(핵심 항목이 하나도 없으면 미작성, 일부만 채우면 작성중, 핵심 항목을 모두 채우면 작성완료), 블록을 누르면 해당 작성 단계로 바로 이동합니다. 배포 주소·GitHub 주소·시연 영상 주소를 넣으면 한 줄 소개 바로 아래에 <strong>🌐 바로 사용하기 💻 소스코드 ▶️ 시연 보기</strong> 형태의 아이콘 링크 배지로 표시되고, 클릭하면 새 탭에서 열립니다(주소에 <code>https://</code>가 없으면 자동으로 붙습니다). <strong>[최종 README]</strong> 탭에서는 문서를 그대로 미리보고 <strong>Markdown 원본 열기·원본 복사·.md 내려받기·인쇄·PDF</strong>를 쓸 수 있습니다. 인쇄를 누르면 편집 화면은 빠지고 <strong>README 본문만 A4로</strong> 출력됩니다. </li>
          <li><strong>활동기록 – 08 사례집 출력</strong>: 지금까지 쓴 내용을 <strong>A4 지면으로 조판한 사례집</strong>으로 바로 볼 수 있습니다. 표지(프로젝트명·한 줄 소개·대표 이미지·팀원·링크 배지)에 이어 개요, 결과물과 기술 구성, 문제 정의 과정, 개발 기록, 교육적 점검, 안전 점검, 제작자와 라이선스, 팀원 후기, 윤리 설문 지면이 순서대로 이어지고 <strong>비어 있는 항목은 자동으로 빠집니다</strong>. 위쪽 <strong>[인쇄·PDF 저장]</strong> 버튼을 누르면 편집 화면은 빠지고 사례집 지면만 인쇄되며, 인쇄 대화상자에서 <strong>대상을 &lsquo;PDF로 저장&rsquo;</strong>으로 고르고 <strong>배경 그래픽 옵션을 켜면</strong> 색과 표가 그대로 저장됩니다. 브라우저 인쇄 대화상자의 <strong>머리글 및 바닥글</strong> 옵션을 끄면 PDF 상단·하단의 날짜·URL 줄이 제거되어 여백이 더 깔끔해집니다. 관리자는 <strong>관리자 &gt; 활동기록 현황</strong>에서 <strong>[사례집 합본 인쇄]</strong>로 모든 팀의 사례집을 한 번에 출력할 수 있습니다(팀 수가 많으면 지면을 만드는 데 잠시 걸립니다).</li>
          <li><strong>활동기록 – 기록한 사람 칸</strong>: <strong>02 문제 정의 과정 기록</strong>과 <strong>04 개발 과정 자유기록</strong>의 각 줄에는 <strong>기록한 사람</strong> 입력칸이 있습니다. 팀원이 [추가] 버튼으로 저장 전 양식을 만들면 <strong>내 닉네임이 자동으로 채워지며</strong>, 필요하면 다른 이름으로 고치거나 비워 둘 수 있습니다. 다른 표(핵심 기능·사용 흐름 등)에는 이 칸이 나오지 않으며, 예전에 이름을 적어 둔 줄은 <strong>기록한 사람: 이름</strong> 형태로만 보입니다.</li>
          <li><strong>활동기록 – 사진 90도 회전</strong>: 첨부한 사진이 옆으로 누워 보이면 미리보기 <strong>왼쪽 위의 회전 버튼</strong>을 눌러 <strong>오른쪽으로 90도씩</strong> 돌릴 수 있습니다. 버튼을 여러 번 눌러 각도를 맞춘 뒤 그 줄의 <strong>[저장]</strong>을 눌러야 실제로 반영되며(저장할 때 한 번만 새로 올라갑니다), 저장 후에는 산출물 보기·README·사례집에도 돌아간 사진으로 나옵니다. 회전한 사진은 <strong>1MB 이하 JPG로 다시 저장</strong>되고 파일명 끝에 <strong>(회전).jpg</strong>가 붙습니다.</li>
          <li><strong>활동기록 – 관리자 수정</strong>: 팀원이 아니어도 관리자는 기록을 고칠 수 있습니다. 활동기록 위쪽 안내문의 <strong>[관리자로 수정하기]</strong> 버튼을 눌러 <strong>관리자 비밀번호</strong>를 입력하면 <strong>관리자 권한으로 편집 중</strong> 안내가 뜨면서 모든 단계를 수정할 수 있습니다. 비밀번호가 틀리면 편집이 열리지 않으며, 확인한 권한은 <strong>브라우저 탭을 닫으면 사라집니다</strong>. 활동기록 글 상단의 <strong>[글 정보 수정]</strong>은 제목·작성자·링크를 고치는 버튼인데, 여기에 <strong>관리자 비밀번호</strong>를 입력하면 글 정보를 수정하는 동시에 아래 활동기록 편집 잠금도 함께 풀립니다. 작성자의 닉네임 비밀번호로 통과하면 글 정보만 수정할 수 있고 활동기록 내용은 그대로 잠금 상태로 유지됩니다.</li>
          <li><strong>활동기록 – 점검과 후기</strong>: <strong>04 개발·교육적 점검</strong>에는 개발 과정 자유기록, 중요한 결정, 막힌 지점, AI 사용, AI 오류·검증, 개인정보 처리 기록이 줄 단위로 들어가고, <strong>교육적 태도 점검</strong> 문항은 <strong>원래 그렇게 했어요 · 점검하다 고쳤어요 · 아직이에요 · 우리 프로그램엔 해당 없어요</strong> 중 하나를 고른 뒤 한 줄 설명을 남깁니다(선택하면 바로 저장, 설명은 [저장] 버튼). 같은 단계의 <strong>변화와 위험 점검</strong>은 확인한 범위, 관찰/기대 구분, 예상 위험과 대응, 멈춤 기준, 검증 방법을 적으며 자동 저장됩니다. <strong>05 개인 후기·소회</strong>는 팀원 한 사람당 1개이며 <strong>본인만 쓰고 고칠 수 있고</strong>, 다른 팀원의 후기는 읽기만 됩니다(관리자는 삭제 가능). 두 기기에서 동시에 저장하면 <strong>"먼저 저장했어요"</strong> 안내가 뜨니 최신 내용을 불러온 뒤 다시 저장하세요. <strong>06 윤리 설문</strong>은 <strong>교사 개발자 윤리 자가점검</strong>으로, 학생 성장 최우선·개인정보 보호·책임과 출처 존중·안전한 실험과 검증·역할 경계 인식·공공성·투명성 일곱 가지 원칙에 각각 <strong>5점 만점 별점을 0.5점 단위</strong>로 매기고(별의 왼쪽 절반을 누르면 0.5점), 마지막에 <strong>더하고 싶은 약속을 200자 이내</strong>로 적습니다. 후기와 마찬가지로 <strong>팀원 한 사람당 1건이며 본인만 고칠 수 있고</strong> 다른 팀원 응답은 읽기만 됩니다(관리자는 삭제 가능). 저장한 응답은 최종 README의 <strong>교사 개발자 윤리 자가점검</strong> 블록에 문항별 평균 점수와 추가 약속으로 정리됩니다.</li>
          <li><strong>활동기록 – 작성 현황(블록 이동)</strong>: 내용이 많은 <strong>03 최종 결과물</strong>과 <strong>04 개발·교육적 점검</strong> 단계에는 <strong>작성 현황</strong>이 함께 표시됩니다. 화면이 매우 넓으면 본문 <strong>왼쪽 여백에 고정된 목록</strong>으로, 그보다 좁으면 본문 위쪽의 <strong>접이식 카드</strong>(제목을 누르면 펼침)로 보입니다. 각 블록마다 <strong>미작성 · 작성중 · 작성완료</strong> 상태와 함께 채운 칸 수(예: 3/6) 또는 등록한 줄 수(예: 2건)가 표시되고, 항목을 누르면 그 블록으로 <strong>바로 이동</strong>합니다. 맨 위에는 <strong>전체 진행도</strong>(예: 3/10 작성완료)가 보입니다.</li>
          ...
          <li><strong>작성 템플릿</strong>: 게시판에 따라 글쓰기 화면을 열면 본문에 <strong>미리 정해진 작성 양식</strong>이 채워져 있을 수 있습니다. 관리자가 그 게시판에 등록해 둔 양식이며, 내용을 자유롭게 고치거나 모두 지우고 써도 됩니다. 한 번 지운 양식은 다시 채워지지 않습니다.</li>
      ...
      <Section id="unread" icon={Bell} title="읽지 않은 글 표시">
        ...
        <ul className="ml-5 list-disc space-y-2">
          ...
          <li>
            <strong>한 번에 읽음 처리</strong>: 닉네임을 새로 등록하면 기존 글이 모두 읽지 않은 글로 잡힙니다. 이때
            게시판 목록 상단의 <strong>[전체 읽음 처리]</strong> 버튼을 누르면 사이트의 모든 글이, 각 게시판 목록 상단의{" "}
            <strong>[이 게시판 읽음 처리]</strong> 버튼을 누르면 그 게시판의 글만 한 번에 읽음으로 표시됩니다.
            확인 창에서 한 번 더 확인을 거치며, <strong>되돌릴 수 없습니다.</strong>
          </li>
      ...
      <Section id="admin" icon={Settings} title="관리자 대시보드 접근">
        ...
        <p>
          카테고리 <strong>수정</strong> 창의 <strong>글 작성 템플릿</strong> 항목에서 그 게시판의
          작성 양식을 등록할 수 있습니다. <strong>일반게시판 / 질문 / 투표</strong> 유형별로 각각
          따로 등록하며, 비워두면 사용자는 빈 본문으로 글쓰기를 시작합니다. 등록해 두면 해당
          게시판에서 글쓰기를 열 때 본문에 자동으로 채워지고, 사용자가 자유롭게 고칠 수 있습니다.
        </p>
        <p>
        ...
        <p>
          평가자 명단 카드 안의 <strong>평가 결과 전체 공개</strong> 스위치로 그 게시판의 평균 점수 공개 범위를
          정합니다. 꺼져 있으면(기본값) 평가 권한이 있는 분과 관리자만 평균을 볼 수 있고, 켜면 모든 방문자에게
          평균 점수가 공개됩니다. 보통 평가 마감 후에 켭니다.
        </p>
        <p>
        ...
        <p>
          <strong>활동기록 현황</strong> 메뉴에서는 도전형 활동기록 게시판을 선택해 팀별
          작성 현황을 한눈에 확인할 수 있습니다. 팀 이름, 팀원, 최종결과물 작성 칸 수, 핵심 기능·사용
          흐름·한계·계획·문제 정의·개발 과정·교육적 점검 항목 수, 개인 후기 작성 인원 등을
          표로 확인할 수 있으며, 행을 펼치면 팀원별 후기 작성 여부도 보입니다. 상단의
          <strong>엑셀</strong> 버튼으로 팀별 요약 시트를, <strong>ZIP</strong> 버튼으로
          각 팀의 상세 README 폴더 전체를 한 번에 내려받을 수 있습니다. ZIP 안의 각 팀 폴더에는
          팀원, 최종결과물, 과정 기록, 점검, 개인 후기, 팀원 약속이 정리된 README.md 파일이
          들어 있어 원료로 활용하기 좋습니다.
        </p>
