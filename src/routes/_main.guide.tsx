    ...
    <div className="space-y-8">
      ...
      <Section id="posting" icon={LayoutGrid} title="글쓰기와 공지">
        ...
        <ul className="ml-5 list-disc space-y-2">
          ...
          <li><strong>문제ZIP 게시판</strong>: 현장의 문제를 세 단계 폼으로 간단히 제보하는 게시판입니다. ① <strong>고통 영역</strong> 선택(예: 보건/건강, 행정/공문, 수업/평가, 학부모소통, 학교행사) — 목록에 없으면 <strong>직접 입력</strong>도 가능합니다, ② <strong>발생 빈도</strong> 선택(매일 / 주 1~2회 / 시즌 한정), ③ <strong>한 줄로 알려주세요</strong>(최대 50자)만 입력하면 됩니다. 제보된 문제는 영역·빈도 배지와 함께 카드 목록으로 공개되며, <strong>공감(하트)</strong>과 <strong>댓글</strong>로 함께 이야기할 수 있습니다. 영역·빈도 선택지는 관리자가 <strong>관리자 &gt; 문제ZIP 선택지</strong>에서 편집합니다.</li>
