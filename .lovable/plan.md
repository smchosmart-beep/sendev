...
- **`getMyReview` 비밀번호 필수화에 따른 UX 보호**: 글 상세 화면에서 닉네임만 입력했을 때 이전 제출 점수를 자동으로 불러오는 기능이 있습니다. 비밀번호가 비어 있으면 쿼리를 `enabled: false`로 두고, 실패는 토스트 없이 조용히 무시해 미입력 상태로 둡니다. queryKey에는 비밀번호를 넣지 않습니다.
- **`listMyReviewedPostIds`는 점수가 아닌 참여 여부만 반환하므로 현행 유지**: 이 함수는 "내가 평가를 제출한 postId 목록"만 주고 구체적인 점수는 포함하지 않아 요약/개별 점수 유출 우려가 없습니다. 다만 계획 범위에서 명확히 하기 위해 별도로 언급합니다.
