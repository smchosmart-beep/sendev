    ...
    <div className="space-y-8">
      ...
      <Section id="posting" icon={LayoutGrid} title="글쓰기와 공지">
        ...
        <ul className="ml-5 list-disc space-y-2">
          ...
          <li><strong>투표 게시판</strong>: 후보 글을 올리고 참여자들이 투표하는 게시판입니다. 글은 <strong>한 닉네임당 1개</strong>만 등록할 수 있고, 등록한 글이 곧 투표 후보가 됩니다. 목록은 <strong>보드형 카드</strong>로 한 화면에 최대 <strong>36개</strong>까지 보이며 그 이상은 페이지로 넘겨 봅니다. <strong>투표 권한</strong>은 그 게시판의 비밀번호를 알고 들어온 사람 누구나 가지며, 투표하려면 <strong>닉네임과 닉네임 비밀번호</strong>가 등록되어 있어야 합니다(메뉴에서 등록). 투표가 <strong>진행 중일 때는 득표수가 모두 비공개</strong>이고 본인이 어디에 투표했는지만 카드에 표시됩니다. <strong>관리자</strong>는 목록 위 제어판에서 <strong>1인당 최대 투표 수</strong>를 정해 <strong>투표 시작</strong>을 누르고, <strong>투표 종료</strong>를 누르면 그 순간 모든 참여자에게 <strong>득표수와 순위</strong>가 한 번에 공개됩니다(1분 이내 자동 반영). <strong>초기화</strong>를 누르면 그 게시판의 표가 모두 삭제됩니다. 이미 투표한 카드를 다시 누르면 투표가 취소되어 다른 후보에게 옮길 수 있습니다.</li>
