...
- 점검 8항목: 8개 행이 고정으로 보이고, 라디오 3택 + 메모 한 줄. 처음 열면 빈 상태로 보이고 고를 때 저장됩니다. 클라이언트는 8개 문항을 미리 렌더링하지만, 서버에서 `kind = 'check'`인 행에 대해서만 `(post_id, sort_order)` **부분 유니크 인덱스**로 중복 생성을 방지합니다. 동시 클릭 등으로 인덱스 충돌이 발생하면 `saveRecordRow`에서 `upsert`/`onConflict` 경로로 기존 행을 갱신하거나, 중복 에러를 사용자에게 "다른 팀원이 먼저 저장했어요" 안내로 처리합니다. 기존 1차 섹션의 `sort_order` 재사용과 충돌하지 않습니다.
