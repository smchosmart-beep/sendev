    ...
    <div className="space-y-8">
      ...
      <Section id="posting" icon={LayoutGrid} title="글쓰기와 공지">
        ...
        <ul className="ml-5 list-disc space-y-2">
          ...
          <li><strong>활동기록 – 단계별 작성 순서</strong>: 기록은 <strong>01 팀 공통정보 → 02 문제 정의 과정 → 03 최종 결과물 → 04 개발·교육적 점검 → 05 개인 후기·소회 → 06 윤리 설문 → 07 README 출력 → 08 사례집 출력</strong>의 8단계로 나뉘어 있고, 위쪽 단계 버튼이나 아래 [이전 단계]·[다음 단계] 버튼으로 오갈 수 있습니다. 각 단계는 따로 저장되므로 <strong>한 번에 다 쓰지 않아도 됩니다</strong>. 각 입력칸에 흐리게 보이는 <strong>회색 예시 문구</strong>는 어떻게 쓰면 되는지 알려 주는 참고용이며, <strong>실제로 저장되지 않습니다</strong>(직접 입력한 내용만 저장돼요). <strong>01 팀 공통정보</strong>에서는 팀원을 추가·삭제하고 각자의 <strong>소속과 역할</strong>을 적습니다. <strong>02 문제 정의 과정</strong>은 회의·인터뷰 같은 <strong>기록 종류</strong>를 고르고 언제·어디서, 나눈 이야기, 그래서 정한 것을 줄 단위로 남깁니다. <strong>모바일에서는 기록 종류를 드롭다운 메뉴로, PC에서는 탭 버튼으로 고릅니다.</strong> 각 줄에는 <strong>관련 링크</strong>(회의록·설문 등 주소 한 줄)와 <strong>관련 파일</strong>(최대 3개, 개당 3MB)을 함께 붙일 수 있습니다. 파일은 고르는 즉시 올라가고 파일명 칩으로 보이며, 칩을 누르면 내려받고 X로 뺄 수 있습니다. 링크·파일을 바꾼 뒤에는 그 줄의 [저장]을 눌러야 확정됩니다. 다만 <strong>기록 종류(탭)에 따라 입력 양식이 달라집니다</strong>. <strong>[22일 팀빌딩·문제 정의 회의]</strong> 탭을 고르면 <strong>페르소나 이미지 첨부 · 고객 여정 맵 이미지 첨부 · 종이 프로토타입 유튜브 영상 링크</strong> 세 칸만 나오고, 이미지 첨부 위에는 <strong>템플릿 안내 블록</strong>이 보이며 오른쪽 <strong>[복사]</strong> 버튼으로 내용을 클립보드에 담아 쓸 수 있습니다. 이미지는 점선 영역에 <strong>끌어다 놓거나 클릭</strong>해서 첨부할 수 있고, 올리면 <strong>본문 너비에 맞춰 크게</strong> 미리보기로 보이고, 유튜브 링크는 입력칸 오른쪽에 <strong>재생 버튼</strong>만 표시되며, 버튼을 누르면 새 탭에서 영상을 바로 볼 수 있습니다. README·사례집 출력에서도 <strong>페르소나 → 고객 여정 맵 → 유튜브 링크</strong> 순서로, 이미지는 실제 그림으로 크게 실립니다. <strong>[인터뷰 기록]</strong> 탭을 고르면 <strong>인터뷰 질문 · 인터뷰 대상 · 인터뷰 내용</strong> 세 칸이 순서대로 나오고, 질문과 내용은 여러 줄로 길게 쓸 수 있습니다(칸당 최대 8,000자, 입력칸 아래에 글자수가 표시됩니다). <strong>[인터뷰 후 문제 구체화 회의]</strong> 탭을 고르면 <strong>가설과 일치했던 내용 · 가설과 불일치했던 내용 · 새로 알게 된 내용</strong> 세 칸이 순서대로 나오며, 세 칸 모두 여러 줄로 길게 쓸 수 있습니다(칸당 최대 8,000자). 나머지 탭은 지금까지의 5칸 양식 그대로입니다. 새 줄을 만드는 <strong>[+ 그 밖의 문제 정의 메모 추가]</strong> 버튼은 <strong>[그 밖의 문제 정의 메모]</strong> 탭을 선택했을 때만 나타나며, 이 버튼으로 추가하면 기록 종류가 <strong>[그 밖의 문제 정의 메모]</strong>로 자동 선택됩니다(필요하면 다른 탭으로 바꿀 수 있어요). <strong>전용 양식 탭</strong>을 선택했는데 아직 기록이 없으면, 편집 권한이 있을 때 <strong>바로 빈 양식 한 벌이 펼쳐집니다</strong>. 내용을 채우고 [저장]을 눌러야 실제 기록으로 남고, 아무것도 입력하지 않은 채 다른 탭으로 넘어가면 저장되지 않습니다. 내용이 하나도 없는 행은 README·사례집 출력과 작성 현황 계산에서 자동으로 빠집니다. <strong>03 최종 결과물</strong>은 한눈에 보기·문제와 해결·사용과 배포·기술 구성·라이선스 항목으로 이루어져 있고, 입력하면 <strong>약 1초 뒤 자동 저장</strong>되며 오른쪽 위에 저장 상태가 표시됩니다. 같은 단계의 <strong>핵심 기능 · 사용 흐름 · 현재 한계 · 다음 계획 · 제작자</strong>는 줄 단위로 추가·수정·삭제하고 [저장] 버튼으로 각 줄만 저장합니다. <strong>대표 이미지는 파일 업로드로 등록</strong>합니다([이미지 선택] 버튼으로 파일을 고르면 자동으로 올라가고 미리보기가 보이며, [제거]로 비울 수 있습니다). <strong>환경변수는 이름만</strong> 적고 실제 값(비밀키)은 절대 입력하지 마세요.</li>
