...
- `src/routes/_main.board.$slug.$postNo.tsx`
  - `post.type === "record"`일 때 헤더 버튼 라벨과 게이트 다이얼로그 설명 문구 분기.
  - `editGateMutation.onSuccess`에서 입력값을 `isRecordAdmin`(`src/lib/record.functions.ts`)으로 한 번 검증 → 관리자면 `setAdminPassword(값)` 호출. 팀원/작성자 비번이면 아무 변화 없음.
- `src/lib/admin-auth.ts`
  - `setAdminPassword`에서 `window.dispatchEvent(new Event("admin-password-changed"))`를 함께 발생(저장 방식·서버 검증은 그대로).
- `src/components/RecordEditor.tsx`
  - 해당 이벤트를 구독해 `adminPw` 상태만 갱신 → `canEdit`/`isAdminEditing` 즉시 반영. 그 외 로직 변경 없음.
