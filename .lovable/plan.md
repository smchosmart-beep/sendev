...
## 보완 9 — 활동기록 글 이동 금지 원칙 (필수)

활동기록 글은 `posts.type = 'record'`이므로 기존 `movePost` 서버 함수가 `"글 게시판 글만 이동할 수 있어요."`로 거부합니다. 활동기록 기능은 이 제약을 그대로 따르며, 추가로 `parent_post_id` 체인(시리즈)에도 연결하지 않습니다. 따라서:

- 카테고리 이동, 시리즈 이동, `move_post_chain` 등 기존 이동 로직에 활동기록 글이 전달되지 않습니다.
- 이동 관련 DB 함수 수정이나 중복 충돌 사전 검사는 불필요합니다.
