    ...
    <div className="space-y-8">
      ...
      <Section id="posting" icon={LayoutGrid} title="글쓰기와 공지">
        ...
        <ul className="ml-5 list-disc space-y-2">
          ...
          <li><strong>활동기록 게시판</strong>: 팀이 한 학기(또는 한 프로젝트)의 과정을 하나의 기록에 함께 정리하는 게시판입니다. <strong>팀당 기록은 1개</strong>이며, 한 게시판에서 한 사람은 <strong>한 팀에만</strong> 속할 수 있습니다. 이미 팀원으로 등록된 사람이 새 기록을 만들려고 하면 새로 만들지 않고 <strong>기존 기록으로 이동</strong>합니다. 기록을 만든 뒤 <strong>팀원 닉네임을 추가</strong>하면 팀원 모두가 같은 기록을 <strong>함께 편집</strong>할 수 있습니다(각자 자기 닉네임과 닉네임 비밀번호로 인증, 메뉴 &gt; 내 정보에 저장해 두면 자동 적용). <strong>최종 결과물</strong> 항목(서비스 이름, 한 줄 소개, 대상, 문제, 해결, 대표 이미지 주소, 배포·GitHub 주소, 사용한 도구, 환경변수 이름)은 입력하면 <strong>약 1초 뒤 자동 저장</strong>되고 화면 오른쪽 위에 저장 상태가 표시됩니다. <strong>핵심 기능 · 사용 흐름 · 지금의 한계 · 다음 계획 · 제작자</strong>는 줄 단위로 추가·수정·삭제하며 [저장] 버튼으로 각 줄만 저장합니다. 다른 팀원이 먼저 같은 곳을 고쳤다면 <strong>"다른 팀원이 먼저 수정했어요"</strong> 안내가 뜨고 최신 내용을 다시 불러옵니다. <strong>환경변수는 이름만</strong> 적고 실제 값(비밀키)은 절대 입력하지 마세요. 활동기록 글은 <strong>다른 카테고리로 이동할 수 없고</strong> 연재(다음 편) 연결도 하지 않습니다. 정리한 내용은 <strong>[README 작성]</strong> 페이지에서 마크다운으로 옮겨 <strong>README.md 또는 ZIP(README + 대표 이미지)</strong>으로 내려받을 수 있습니다. 게시판에서 활동기록을 켜고 이름을 바꾸는 것은 <strong>관리자 &gt; 카테고리 관리</strong>에서 합니다.</li>
          <li><strong>활동기록 – 과정 · 점검 · 후기</strong>: 최종 결과물 아래에 네 가지 영역이 더 있습니다. <strong>문제 정의 과정 기록</strong>(단계/날짜, 우리가 나눈 이야기, 그래서 정한 것)과 <strong>개발 과정 자유기록</strong>(날짜, 무슨 일이 있었나, 어떻게 해결했나)은 줄 단위로 추가하며, 긴 칸은 <strong>2,000자</strong>까지 쓸 수 있고 입력창 아래에 글자 수가 표시됩니다. <strong>교육적 점검 8항목</strong>은 고정된 8개 질문에 <strong>잘함 · 보통 · 아직</strong> 중 하나를 고르고 한 줄 메모를 남깁니다(선택하는 즉시 저장, 메모는 [저장] 버튼). <strong>개인 후기와 약속</strong>은 팀원 한 사람당 1개이며 <strong>본인만 쓰고 고칠 수 있고</strong>, 다른 팀원의 후기는 읽기만 됩니다(관리자는 삭제 가능). 두 기기에서 동시에 저장하면 <strong>"먼저 저장했어요"</strong> 안내가 뜨니 최신 내용을 불러온 뒤 다시 저장하세요.</li>
